...
public class VampireShield : MonoBehaviour {
	...
	public PlayerEnum owningPlayer {
		...
		set {
			...
			if (GameManager.S.inGame) {
				hitboxOffset = GameManager.S.players[(int)value].character.transform.FindChild("Hitbox").localPosition.y;
			}
	...
	public void ActivateShield() {
		...
		if (thisPlayer.character is Vampire) {
			(thisPlayer.character as Vampire).vShip.shieldUp = true;
		}
	...
	void DestroyShield() {
		...
		if (thisPlayer.character is Vampire) {
			(thisPlayer.character as Vampire).vShip.shieldUp = false;
		}
		if (thisPlayer.durationBar != null) {
